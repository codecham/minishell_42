...
/*   Updated: 2022/11/07 03:17:35 by dcorenti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

int	ft_insert_red_file_name(t_data_parsing *p, t_token *token, t_token *file_name)
{
	if (token->type != HEREDOC && ft_is_empty_token(file_name) == 1)
		return (ft_err_pars_message(p, ": No such files or drectory", -2));
	token->red_file_name = ft_strdup(file_name->value);
	if (!token->red_file_name)
		return (-1);
	ft_delete_token_in_list(p, file_name);
	return (0);
}
